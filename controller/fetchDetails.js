...
export const updateDetails = async (req, res) => {
  try {
    ...
    Synonymns = (await Synonymns.includes("\n"))
      ? Synonymns?.split("\n")
      : Synonymns;
    Classification = (await Classification.includes("\n"))
      ? Classification?.split("\n")
      : Classification;
    Morphology = (await Morphology.includes("\n"))
      ? Morphology?.split("\n")
      : Morphology;
    Rasapanchaka = (await Rasapanchaka.includes("\n"))
      ? Rasapanchaka?.split("\n")
      : Rasapanchaka;
    Amayika_Prayoga = (await Amayika_Prayoga.includes("\n"))
      ? Amayika_Prayoga?.split("\n")
      : Amayika_Prayoga;
    Dose = (await Dose.includes("\n")) ? Dose?.split("\n") : Dose;
    Yoga_formulation = (await Yoga_formulation.includes("\n"))
      ? Yoga_formulation?.split("\n")
      : Yoga_formulation;
    Shlok = (await Shlok.includes("\n")) ? Shlok?.split("\n") : Shlok;
    const result = await DrugDetailsModel.findOneAndUpdate(
      { _id: id },
      {
        drug_name: drug_name,
        Description: Description,
